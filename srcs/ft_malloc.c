...
void	alloc_next_zone(t_page **page, size_t zone_size, t_block **p, int index)
{
...
void	*new_zone(size_t index, size_t alloc_size, size_t zone_size)
{
	...
	if (p == NULL || (alloc_size > (char *)(page + 1) + zone_size - (char *)(p + 1) + alloc_size + sizeof(t_block *)))
	{
		alloc_next_zone(&page, zone_size, &p, index);
